        ...
        <div className="space-y-4">
          ...
          {isPlan && (
            <div className="bg-gray-50 p-3 rounded border">
              <div className="grid grid-cols-2 gap-3 text-sm">
                <label className="flex flex-col">
                  <span className="text-xs text-gray-600">Plazo (días) (opcional)</span>
                  <input type="text" inputMode="numeric" pattern="[0-9]*" value={plazo}
                    onChange={(e)=>{
                      const v = e.target.value.replace(/[^0-9]/g,'');
                      setPlazo(v);
                ...
                <label className="flex flex-col">
                  <span className="text-xs text-gray-600">Monto reserva</span>
                  <input type="text" inputMode="decimal" pattern="[0-9]*[.,]?[0-9]*" value={abono}
                    onChange={(e)=>{
                      let v = e.target.value.replace(/[^0-9.,]/g, '');
                      const parts = v.split(/[.,]/);
              ...
              <div className="mt-3 text-xs text-gray-600">La reserva sólo guarda este monto en la tabla reservación. No se generan intereses ni cuotas.</div>
            </div>
          )}
            ...
            <button
              onClick={()=>{
                ...
                if (localTipo==='credito') {
                  ...
                  if(setAnticipoReserva) setAnticipoReserva(0); // Crédito no usa anticipoReserva directo
                } else if(localTipo==='plan') {
                  // Sólo usamos abonoInicial como monto_reserva y el plazo para fecha_evento
                  setCreditoConfig({ plazoDias: plazoNum, abonoInicial: Math.min(abonoNum, Number(total)||0), interesPorc: 0, numCuotas: 1 });
                  if(setAnticipoReserva) setAnticipoReserva(Math.min(abonoNum, Number(total)||0));
                } else {
                  setCreditoConfig({ plazoDias: 0, abonoInicial: 0, interesPorc:0, numCuotas:1 });
                  if(setAnticipoReserva) setAnticipoReserva(0);
                }
                onClose();
              }}
