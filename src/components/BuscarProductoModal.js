...
const BuscarProductoModal = ({ isOpen, onClose, onSelect }) => {
  ...
  const load = async (q = '') => {
    ...
    try {
      let res;
      if (q.trim() === '') res = await controller.getAllProductos();
      else res = await controller.searchProductos(q, 'producto');
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-start justify-center bg-black bg-opacity-40 p-6" onClick={(e)=>e.target===e.currentTarget && onClose()}>
      <div className="bg-white w-full max-w-3xl rounded shadow-lg flex flex-col border border-gray-500">
        <div className="px-3 py-2 border-b bg-gray-200 flex items-center space-x-2">
          <span className="font-semibold text-sm">Buscar Productos</span>
          <input
            autoFocus
            value={query}
          ...
          <table className="w-full text-sm border-collapse">
            <thead className="bg-gray-100">
              <tr>
                ...
                <th className="text-left px-2 py-1 border">Descripción</th>
                <th className="text-right px-2 py-1 border">P. Compra</th>
                <th className="text-right px-2 py-1 border">Stock</th>
              </tr>
            </thead>
            <tbody>
              {items.map((p, idx) => (
                ...
                >
                  <td className="px-2 py-1 border whitespace-nowrap">{p.codigo}</td>
                  <td className="px-2 py-1 border">{p.producto}</td>
                  <td className="px-2 py-1 border text-right">{(p.pcompra||0).toFixed? (p.pcompra||0).toFixed(2): p.pcompra}</td>
                  <td className="px-2 py-1 border text-right">{p.almacen}</td>
                </tr>
              ))}
            </tbody>
          </table>
